...
export default function Claim() {
        ...
        return (
          ...
          >
            {swapEndian(record.data[0])
              ?.substring(0, 7)
              .concat('...')
              .concat(swapEndian(record.data[0])?.substring(swapEndian(record.data[0]).length - 5))}
          </a>
        )
      },
    },
  ...
  return <ChainXTable Columns={chainColumns} urlControl={'/crossblocks/bitcoin/unclaim?'} result={'items'} keyNum={7} />
}
