  ...
  return (
      <ChangeNetDiv className={`${name ? 'border-white' : 'border-blue'}`} >
        <div className={`${name ? 'flex flex-row border border-gray-white' : 'flex flex-row border-gray-arrow border'}`} onMouseEnter={onMouseEnterUl}>
          ...
          <span className={name ? 'text-white-light px-2 min-w-12 text-center' : 'text-topBar-blue px-2 min-w-12'}>{name ? 'TESTNET' : 'MAINNET'}</span>
          {/*<img src={name ? pullB : pullW} alt="select" className="img2" />*/}
        </div>
        { <ul className={["flex flex-col", test ? 'no-show' : ''].join(' ')}>
          <li className={name ? '' : 'bg-topBar-light'} onClick={() => changeFun(false)}><span>MAINNET</span></li>
